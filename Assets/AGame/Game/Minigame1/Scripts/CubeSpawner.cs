...
public class CubeSpawner : MonoBehaviour
{
    ...
    private float boxZDistanceFromCamera = 15f;
    private Transform _boxTransform;
    
    [SerializeField] private Camera _camera;

    private Vector3 boxBoundsForGizmoz;
    
    void Start()
    {
        Vector3 boxPosition = _camera.transform.position + _camera.transform.forward * boxZDistanceFromCamera;
        Vector3 boxBounds = CalculateCameraBounds(_camera);
        boxBoundsForGizmoz = boxBounds;
    ...
    {
        Vector3 screenBottomLeft = camera.ViewportToWorldPoint(new Vector3(0f, 0f, boxZDistanceFromCamera));
        Vector3 screenTopRight = camera.ViewportToWorldPoint(new Vector3(1f, 1f, boxZDistanceFromCamera));
    ...
    private void SetupBoundaryBox(Vector3 boxPosition, Vector3 boxBounds)
    {
        ...
        //CreateWall("Wall_Front", new Vector3(0, 0, hz + thickness / 2f), new Vector3(boxBounds.x + thickness, boxBounds.y + thickness, thickness), box.transform);
        //CreateWall("Wall_Back", new Vector3(0, 0, -hz - thickness / 2f), new Vector3(boxBounds.x + thickness, boxBounds.y + thickness, thickness), box.transform);
    }
    ...
    {
        if(boxBoundsForGizmoz == Vector3.zero) return;
        Vector3 gizmoBounds = boxBoundsForGizmoz;
        
        Gizmos.color = Color.yellow;
        Gizmos.DrawWireCube(transform.position, gizmoBounds);
        
        Vector3 visibleBounds = new Vector3(gizmoBounds.x - 2 * padding, gizmoBounds.y - 2 * padding, gizmoBounds.z);
        Gizmos.color = Color.green;
        Gizmos.DrawWireCube(transform.position, visibleBounds);
        
        Gizmos.color = new Color(1f, 0.5f, 0f, 0.5f);
        Gizmos.DrawWireCube(transform.position, gizmoBounds + Vector3.one * wallThickness);
    }
}